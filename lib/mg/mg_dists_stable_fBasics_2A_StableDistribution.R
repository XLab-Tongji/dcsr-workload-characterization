...
source("lib/mg/mg_math_integral.R"); #[sguazt]
...
{   # A function implemented by Diethelm Wuertz
    ...
    if (alpha == 1 & abs(beta) <= 1 & beta != 0) {
        ...
        function(x, xarg, alpha, beta) {
            ...
            gval = g * exp(-g) 
            # replace NA at pi/2
            #for (i in 1:length(gval)) if(is.na(gval[i])) gval[i] = 0 # [sguazt]: commented
            gval[is.na(gval)] <- 0; # [sguazt]
            gval 
        }
        # Integration:  
...
{   # A function implemented by Diethelm Wuertz
    ...
    if (abs(alpha-1) < 1) {
        .froot <- function(x, alpha, beta, subdivisions, p) {
            return( pstable( q = x, alpha = alpha, beta = beta, pm = 0) - p );
        }
        # Calculate:
        result = rep(NA, times = length(p))
        for (i in 1:length(p)) {
...
{   # A function implemented by Diethelm Wuertz
    ...
    if (class(version) != "Sversion") {
        ...
        #END commented [sguazt]
        #ans <- integrate(f, lower, upper, subdivisions, rel.tol, abs.tol, stop.on.error = FALSE, ...); #[sguazt]
	ans <- mg_math_integral.gauss2c( f, lower, upper, subdivisions, ... );
    } else {
        # SPlus:
